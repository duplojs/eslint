...
function getLineIndentFromToken(src: Rule.RuleContext["sourceCode"], tok: Token): string {
...
export const wrapParensIntersection: Rule.RuleModule = {
    meta: {
        ...
        docs: {
            description:
                "For parenthesized intersection types, wrap the members on separate lines with one '&' per line and place the closing parenthesis on its own line.",
        },
        fixable: "code",
        schema: [],
        messages: {
            wrap: "Wrap parenthesized intersection members across multiple lines.",
        },
    },
    create(context) {
        ...
        function handleIntersection(node: Node & Rule.NodeParentExtension & { types?: Node[] }) {
            ...
            const firstType = types[0] as Node;
            const lastType = types[types.length - 1] as Node;

            const firstTok = src.getFirstToken(firstType as any) as Token | null;
            if (!firstTok) return;
            const prevTok = src.getTokenBefore(firstTok) as Token | null;
            if (!prevTok || prevTok.value !== "(") return; // only handle parenthesized intersections
            const open = prevTok;
            ...
            const currentAfterOpen = src.text.slice(afterOpenRange[0], afterOpenRange[1]);
            ...
            const currentBeforeClose = src.text.slice(beforeCloseRange[0], beforeCloseRange[1]);
            const desiredBeforeClose = `\n${baseIndent}`;

            const fixes: Array<(fixer: Rule.RuleFixer) => any> = [];

            if (currentAfterOpen !== desiredAfterOpen) {
                fixes.push((fixer) => fixer.replaceTextRange(afterOpenRange, desiredAfterOpen));
            }
            if (currentBeforeClose !== desiredBeforeClose) {
                fixes.push((fixer) => fixer.replaceTextRange(beforeCloseRange, desiredBeforeClose));
            }
