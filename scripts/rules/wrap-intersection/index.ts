...
export const wrapIntersection: Rule.RuleModule = {
    meta: {
        type: "layout",
        docs: {
            description: "Wrap intersection types so each '&' begins a new line.",
        },
        fixable: "code",
        schema: [],
        messages: {
            wrap: "Wrap intersection members with line breaks before '&'.",
        },
    },
    create(context) {
        const src = context.sourceCode;

        function handleIntersection(node: Node & { types?: Node[] }) {
            ...
            const current = src.text.slice(start, end);
            ...
            context.report({
                ...
                fix(fixer) {
                    if (prevTok && prevTok.value === "=") {
                        // Replace from just after '=' to the end of the intersection
                        return fixer.replaceTextRange([prevTok.range![1], end], ` ${desired}`);
                    }
                    return fixer.replaceTextRange([start, end], desired);
                },
            });
        }
