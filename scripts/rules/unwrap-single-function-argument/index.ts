...
function hasUnionOrIntersection(
    param: Param,
    getText: (node: Node) => string,
): boolean {
    const txt = getText(param);
    ...
    for (let i = 0; i < typeText.length; i++) {
        const ch = typeText[i];
        if (inString) {
            if (ch === inString && typeText[i - 1] !== "\\") inString = null;
            continue;
        }
        if (ch === '"' || ch === "'" || ch === "`") {
            inString = ch;
...
function textHasUnionOrIntersection(typeText: string): boolean {
    let inString: string | null = null;
    for (let i = 0; i < typeText.length; i++) {
        const ch = typeText[i];
        if (inString) {
...
export const unwrapSingleFunctionArgument: Rule.RuleModule = {
	...
	create(context) {
        ...
        function handleFunction(node: (FunctionDeclaration | ArrowFunctionExpression) & Rule.NodeParentExtension): void {
            ...
            // Do not unwrap when parameter type contains union or intersection
            const innerText = currentText.slice(1, -1);
            if (textHasUnionOrIntersection(innerText)) {
                return;
            }
